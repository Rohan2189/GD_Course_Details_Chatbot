...
def processRequest(req):
    ...
    if (intent=='course_selection'):
        ...
        if collection['SessionID']== sessionID:
            collection.update_one({"SessionId": sessionID}, {"$push": {"Event": {"$each": [user_says, bot_says]}}},
                                 True)
        else:
            collection.insert_one(
            {"SessionId": sessionID, "Name": cust_name, "Contact": cust_contact, "EmailId": cust_email,
             "Course": course_name,"Events":[user_says,bot_says]})
